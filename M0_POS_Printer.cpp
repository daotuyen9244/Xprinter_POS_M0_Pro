...
// Constructor
ESC_POS_Printer::ESC_POS_Printer(Stream *s) : stream(s)
{
}
...
void ESC_POS_Printer::setBarcodeHeight(uint8_t val)
{ // Default is 50
    if (val < 1)
        val = 1;
    barcodeHeight = val;
...
void ESC_POS_Printer::printEndfile()
{
    stream->write('\n');
    stream->write('\n');
    stream->write('\n');
